...
public class TotalGodDataLowMMR extends TotalGodData {
    ...
    @CollectionTable(name = "total_skins_low_mmr")
    private Map<String, Integer> skinsUsed;

    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "total_items_low_mmr")
    private Map<Item, Integer> popularItems;

    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "total_new_patch_items_low_mmr")
    private Map<Item, Integer> newPatchPopularItems;

    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "total_actives_low_mmr")
